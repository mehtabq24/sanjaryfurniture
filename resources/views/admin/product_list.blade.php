...
<th class="crancy-table__column-1 crancy-table__h1">Product</th>
<th class="crancy-table__column-4 crancy-table__h4">Category</th>
<th class="crancy-table__column-6 crancy-table__h6">Discount</th>
<th class="crancy-table__column-7 crancy-table__h7">Status</th>
<th class="crancy-table__column-8 crancy-table__h8">Edit</th>
<th class="crancy-table__column-8 crancy-table__h8">View</th>
</tr>
</thead>

{{-- @foreach ($productsData as $product)
    <div class="product-card">
        <img src="{{ asset('images/' . ($product->frontImage ?? 'default.jpg')) }}" 
             alt="{{ $product->productName }}" class="product-image">
        <h3>{{ $product->productName }}</h3>
        <p>Category: {{ $product->productCategory }}</p>
        <p>Discount: {{ $product->productDisc }}%</p>
        <p>Status: {{ ucfirst($product->productStatus) }}</p>
    </div>
@endforeach --}}
...
<tbody class="crancy-table__body searchproduct">
    ...
    {{-- <img style="width: 150px;" src="{{ asset('images/' . $product->productImage) }}" alt="Image description"> --}}
    </div>
    </td>
    <td class="crancy-table__column-2 crancy-table__data-2">
    <h5 class="crancy-table__inner--title">{{$product->productName}}</h5></td>
    
    <td class="crancy-table__column-4 crancy-table__data-4">
    <h5 class="crancy-table__inner--title">{{ $product->productCategory }}</h5>
    </td>
    <td class="crancy-table__column-3 crancy-table__data-3">
    <p class="crancy-table__text crancy-table__time">{{ $product->productDisc }}</p>
    </td>
    <td class="crancy-table__column-7 crancy-table__data-7">
    <div class="crancy-table__status">{{ $product->productStatus}}</div>
    </td>
    
    <td class="crancy-table__column-6 crancy-table__data-6">
    <div class="crancy-chatbox__edit"><a href="{{ url('update_product', $product->id) }}"><img src="admin/img/edit-icon2.svg"></a></div>
    </td>
    <td class="crancy-table__column-6 crancy-table__data-6">
